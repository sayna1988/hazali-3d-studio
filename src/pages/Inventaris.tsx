...
import { AlertTriangle, Boxes, CircleDollarSign, PackageCheck, Plus, Search, SlidersHorizontal, X } from "lucide-react";
...
export default function Inventaris() {
  ...
  function uploadFoto(event: React.ChangeEvent<HTMLInputElement>) {
  ...
  function resetFormulier() {
    setNaam(""); setSku(""); setVoorraad(1); setMinimumVoorraad(2);
    setKostprijs(0); setVerkoopprijs(0); setLocatie(""); setFoto("");
  }
  ...
  return <div className="inventory-page">
    <header className="inventory-hero"><div><span className="inventory-eyebrow">Productbeheer</span><h1>Inventaris</h1><p>Alles wat klaarstaat om verkocht te worden, helder op één plek.</p></div><button className="inventory-add-button" onClick={() => setFormulierOpen(true)}><Plus size={19} strokeWidth={2.4} />Nieuw product</button></header>
    ...
    <section className="inventory-stats" aria-label="Inventarisoverzicht">
      ...
      <article className="inventory-stat-card"><span className="inventory-stat-icon green"><CircleDollarSign size={20} /></span><div><span>Verkoopwaarde</span><strong>{euro.format(stats.verkoopwaarde)}</strong></div><small>Potentiële omzet</small></article>
    ...
    <section className="inventory-workspace">
      ...
      {zichtbareProducten.length > 0 ? <div className="inventory-grid">{zichtbareProducten.map((product) => <InventoryCard key={product.id} product={product} onIncrease={(id) => pasVoorraadAan(id, 1)} onDecrease={(id) => pasVoorraadAan(id, -1)} onDelete={verwijderen} />)}</div> : <div className="inventory-empty"><span><PackageCheck size={30} /></span><h2>{producten.length === 0 ? "Je inventaris is nog leeg" : "Geen producten gevonden"}</h2><p>{producten.length === 0 ? "Voeg je eerste verkoopklare print toe en houd vanaf hier je voorraad bij." : "Probeer een andere zoekterm of pas je filter aan."}</p>{producten.length === 0 ? <button onClick={() => setFormulierOpen(true)}><Plus size={17} />Eerste product toevoegen</button> : <button onClick={() => { setZoeken(""); setFilter("alles"); }}>Filters wissen</button>}</div>}
    </section>
